...
func makeModelFromString(name string) Model {
	model := Model{}
	model.StructName = stringx.Singular(strcase.ToCamel(name))
	model.StructNamePlural = stringx.Plural(model.StructName)
	model.TableName = stringx.Snake(model.StructNamePlural)
	model.VariableName = stringx.LowerCamel(model.StructName)
	model.PackageName = stringx.Snake(model.StructName)
	model.VariableNamePlural = stringx.LowerCamel(model.StructNamePlural)
...
func createFileFromStub(filePath string, stubName string, model Model, variables ...interface{}) {
	...
	// 存储到目标文件中
	err = filex.Put([]byte(modelStub), filePath)
	if err != nil {
		console.Exit(err.Error())
	}
